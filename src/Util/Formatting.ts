...
export function getAlbumArt(item: BaseItemDto, size: number | null = null) {
...
export function formatSeconds(seconds: number, pretty = false, includeSeconds = true) {
...
export function generateDeviceId() {
  ...
  keys.push(new Date().getTime().toString());
...
export function pluralize(amount: number, singular: string, plural: string | null = null) {
  if (amount === 1) {
    return singular;
  } else {
